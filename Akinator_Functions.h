...
tree_header * File_To_Tree(const FILE * source){
    assert(source);
    char * buf = (char *) File_To_Buf((FILE *)source);
...
elem * File_To_Tree_Mini(char ** buf, tree_header * header){
    ...
    for (char * i = pointer_now; i < space_pointer; ++i) {
        if (*i == ')') {
            element->value[i - pointer_now] = '\0';
            break;
        }
        element->value[i - pointer_now] = *i;
    }

    element->value[space_pointer - pointer_now] = '\0';
    p(9, ("value %s\n", element->value));
